...
def compute_layer0(prices, holdings):
    ...
    for mkt in markets_use:
        ...
        market_summary[mkt] = {
            ...
            "evt_var": round(rm["evt_var"], 3) if not np.isnan(rm["evt_var"]) else round(abs(rm["current_var"]) * 1.35, 3) if not np.isnan(rm["current_var"]) else 0,
            "evt_cvar": round(rm["evt_cvar"], 3) if not np.isnan(rm["evt_cvar"]) else round(abs(rm["current_cvar"]) * 1.35, 3) if not np.isnan(rm["current_cvar"]) else 0,
